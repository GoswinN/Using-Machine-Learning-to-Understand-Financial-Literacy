...
This is the R code that is used in the analysis of the thesis: "Using Machine Learning to Understand Financial Literacy". 

Data on people's finances, work, personality, and social integrated has been collected and is organized, examined, and analysed here.
...
#Housing.12 <- as.data.frame(read_sav("cd12e_EN_1.0p.sav"))
...
#Background.12 <- as.data.frame(read_sav("avars_201201_EN_2.0p.sav"))

#Financial Literacy specific Data
#F.Capability.10 <- as.data.frame(read_sav("dk10a_EN_1.1p.sav"))
...
#Data Examination
The next part examines the data of the selected datasets
```{r}
skim(F.Literacy.11)
```
...
A barrier is set so that 95% of the observations in the data in Assets.12 need to remain in the data set.
...
```{r}
#y is outcome variable: dependent variable (Financial Risk)
...
#Make tables with the results

Diversification
```{r}
table.Diversification <- as.data.frame(cbind(lasso.effects.Diversification$coefficients, lasso.effects.Diversification$se, 
                               lasso.effects.Diversification$t, lasso.effects.Diversification$pval))
```

```{r}
table.Diversification$V4[table.Diversification$V4 <= .001] <- .001
```

```{r}
table.Diversification <- round(table.Diversification, digits = 3)
```

```{r}
table.Diversification$Variables <- rownames(table.Diversification)
```

```{r}
write.csv(table.Diversification, "Diversification_Table.csv")
```


Investments
```{r}
table.Investments <- as.data.frame(cbind(lasso.effects.Investments$coefficients, lasso.effects.Investments$se, 
                               lasso.effects.Investments$t, lasso.effects.Investments$pval))
```

```{r}
table.Investments$V4[table.Investments$V4 <= .001] <- .001
```

```{r}
table.Investments <- round(table.Investments, digits = 3)
```

```{r}
table.Investments$Variables <- rownames(table.Investments)
```

```{r}
write.csv(table.Investments, "Investments_Table.csv")
```


Risk Preferences
```{r}
table.Risk.Final <- as.data.frame(cbind(lasso.effects.Risk.Final$coefficients, lasso.effects.Risk.Final$se, 
                               lasso.effects.Risk.Final$t, lasso.effects.Risk.Final$pval))
```

```{r}
table.Risk.Final$V4[table.Risk.Final$V4 <= .001] <- .001
```

```{r}
table.Risk.Final <- round(table.Risk.Final, digits = 3)
```

```{r}
table.Risk.Final$Variables <- rownames(table.Risk.Final)
```

```{r}
write.csv(table.Risk.Final, "Risk.Final_Table.csv")
```